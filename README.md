...
| 游记图文 | 地点 ↔ 游记一对多，**Tiptap 所见即所得编辑器（V2）**：顶部格式工具栏（H1/H2/H3、加粗/斜体/删除线、列表、引用、链接、分割线、代码块、图片）+ 输入 `/` 打开插入菜单（10 个固定项，↑↓/Enter/Esc 键盘导航）+ 选中文字弹出浮动工具栏（B/I/S/H1/H2/链接/引用）+ **图片直接以预览形式嵌入正文**（上传即显示、可删除/调宽，非图片链接文本）；**Markdown 负责持久化**（blogs.content 仍是 Markdown，react-markdown 阅读页不变），旧 Markdown 文章经 round-trip 兼容测试（7/7 语法无丢失）无缝迁移；多图上传（本地存储），正文图片自动同步进图集（去重）；编辑未保存状态提示（● 有未保存修改 / 离开确认），列表 / 阅读 / 编辑三模式 |
...
- **图表**：recharts
- **富文本**：react-markdown（阅读渲染）+ **Tiptap v3（所见即所得编辑器：@tiptap/react / starter-kit / markdown / image / link，Markdown 持久化）**
...
components/editor/      # Tiptap 所见即所得编辑器（TravelEditor / Toolbar / BubbleMenu / SlashMenu / ImageNode / markdown 转换 / 统一扩展配置）
lib/                   # prisma 单例、auth、validate（输入校验）、工具函数
prisma/                # schema + 迁移文件 + seed 脚本
scripts/               # Markdown 往返兼容测试（test-markdown-roundtrip.mjs）
docs/                  # 数据库设计文档 + 系统测试清单
```
